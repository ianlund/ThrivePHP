<?
	...
	function exceptionHandler($e) {
		...
		// Send email
		mail("error@orange.io", $subject, $subject."\n".$vars."\n".$trace."\n".$globals, "From: error@ipaperbox.com", '-f error@orange.io');
		// Display error
		$displayErrors = (ini_get('display_errors') == '1' || ini_get('display_errors') == "yes" ? true : false);
		if($displayErrors && isset($_SERVER['HTTP_HOST'])) print('<div style="border: 1px solid red; color: red; background: #fcc; padding: 1em;"><pre style="margin: 0;">'."\n".$subject."\n</pre>"."\n<!-- ***** START ERROR *****\n".date("Y-m-d H:i:s", $time)." ".$subject."\n".$vars."\n".$trace."\n".$globals."\n***** END ERROR *****--></div>");
	}
	class UserException extends Exception {
		public function html() {
			$cssClass = 'error';
			if($this->code==1) $cssClass = 'success';
			if($this->code==2) $cssClass = 'warning';
			return '<div class="'.$cssClass.'">'.$this->message.'</div>';
		}
	}
